...
export default function CreateWalletTransaction({
  ...
  const initialDate = transaction?.transaction_date
  ...
  const handleEditWalletTransaction = async ({
    user_id,
    wallet_id,
    category_id,
    value,
  ...
  const handleWalletTransaction = isEditing
    ? handleEditWalletTransaction
    ...
    <Card>
      <Form {...form}>
        <form onSubmit={handleSubmit(handleWalletTransaction)}>
          <CardContent className="flex w-full flex-col gap-2 space-y-2 py-4">
            <div className="flex flex-col items-start justify-center gap-1 space-y-1">
              {errors.wallet_id?.message && (
                <FormError message={errors.wallet_id.message} />
              )}
              <Controller
                control={control}
                name="wallet_id"
                    ...
                    <SelectContent>
                      <SelectGroup>
                        {wallets.map((wallet: Wallet, index: number) => {
                          const walletBank =
                            banks.find((bank) => bank.id === wallet.bank_id) ||
                            banks[0];
            ...
            <div className="flex w-full flex-col items-start justify-center gap-1 space-y-1">
            ...
            <div className="flex flex-col items-start justify-center gap-1 space-y-1">
              {errors.transaction_date?.message && (
                <FormError message={errors.transaction_date.message} />
              )}
              <Popover>
